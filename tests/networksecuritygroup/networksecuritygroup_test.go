...
func TestNetworkSecurityGroup(t *testing.T) {
	...
	parent_id := "/subscriptions/" + v["subscription_id"].(string) + "/resourceGroups/" + v["resource_group_name"].(string)
	...
	check.InPlan(test.PlanStruct).That("azapi_resource.network_security_group").Key("parent_id").HasValue(parent_id).ErrorIsNil(t)
}
