locals {
  # subscription_module_output_subscription_id is either the output of the subsciption module,
  ...
  subscription_module_output_subscription_id = can(module.subscription[0]) ? module.subscription[0].subscription_id : ""

  # subscription_module_output_subscription_id is either the output of the subsciption module,
  # or if disabled, an empty string.
  # Needed to avoid errors in local.subscription_id when referencing a module instance that does not exists.
  subscription_module_output_subscription_resource_id = can(module.subscription[0]) ? module.subscription[0].subscription_resource_id : ""
